<script lang="ts">
	...
	$effect(() => {
		if (voice.activeCall?.status === 'active' && voice.activeCall.answeredAt && !timer) {
			const start = new Date(voice.activeCall.answeredAt).getTime();
			elapsed = Math.floor((Date.now() - start) / 1000);
			timer = setInterval(() => {
				elapsed = Math.floor((Date.now() - start) / 1000);
			}, 1000);
		}
		if (!voice.activeCall && timer) {
			clearInterval(timer);
			timer = null;
			elapsed = 0;
		}
...
{#if voice.activeCall}
	<div class="dm-call-header" role="status" aria-label="Voice call in progress">
		<div class="call-info">
			{#if voice.activeCall.otherAvatarUrl}
				<img class="call-avatar" src={voice.activeCall.otherAvatarUrl} alt="" />
			{:else}
				<div class="call-avatar-placeholder">
					{voice.activeCall.otherDisplayName.slice(0, 1).toUpperCase()}
				</div>
			{/if}
			<div class="call-details">
				<span class="call-name">{voice.activeCall.otherDisplayName}</span>
				{#if voice.activeCall.status === 'ringing'}
		...
		<div class="call-controls">
			{#if voice.activeCall.status === 'active'}
				<button
					class="ctl-btn"
			...
			<button
				...
				aria-label={voice.activeCall.status === 'ringing' ? 'Cancel call' : 'End call'}
			>
