...
- **Discord server import** — full server migration from Discord via bot token; imports server structure (categories, text and voice channels, roles, permission overrides), message history with attachments and replies, custom emojis, and member mappings; newest-first message import so users see recent conversations immediately; live `ImportMessagesAvailable` SignalR events update channels in real-time during import; 4 channels imported concurrently with global `TokenBucketRateLimiter` (50 req/sec); two-phase import: text import completes first (`RehostingMedia` status), then emojis and image attachments are re-hosted to Codec storage via SkiaSharp in the background (non-image file attachments and member avatars continue to use Discord CDN URLs); step-by-step import wizard with bot setup instructions and invite URL generator; identity claiming lets Discord members link their imported messages to their Codec account; one-shot import with incremental re-sync (only new messages fetched via `after` pagination); orphaned import recovery on worker startup (re-queues stuck `RehostingMedia` imports, fails orphaned `InProgress` imports); stale rehost detection in wizard and server settings (30-minute threshold with re-sync prompt); re-sync allowed from `RehostingMedia` state; benchmarked at 371K messages in ~20 minutes
