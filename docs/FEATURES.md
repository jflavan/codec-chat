...
- **Discord server import** — full server migration from Discord via bot token; imports server structure (categories, channels, roles, permission overrides), message history with attachments and replies, custom emojis, and member mappings; step-by-step import wizard with bot setup instructions and invite URL generator; parallel channel imports with Discord API rate limiting; identity claiming lets Discord members link their imported messages to their Codec account; one-shot import with optional re-sync for new messages; real-time progress via SignalR
