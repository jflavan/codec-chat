...
3. **Video embeds:** Should YouTube/Vimeo links get special treatment with inline video players? — **Partially resolved.** For YouTube, links are detected client-side via `extractYouTubeVideoId()` in `$lib/utils/youtube.ts` and rendered as inline `<iframe>` players (`YouTubeEmbed.svelte`) using the privacy-enhanced `youtube-nocookie.com` domain. Vimeo support remains a follow-up and can be added similarly in the future.
