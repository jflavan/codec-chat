...
1. **Caching:** ~~Should fetched metadata be cached to avoid re-fetching the same URL across messages?~~ — **Resolved.** `LinkPreviewService.FetchMetadataAsync` checks Redis (`IDistributedCache`) before making HTTP requests. Cache key is `linkpreview:{SHA256(url)}` with a 1-hour TTL. Failed fetches are cached too (as a `__failed__` sentinel) to avoid re-hitting broken URLs. Redis failures degrade gracefully — the service falls through to a normal fetch.
...
3. **Video embeds:** Should YouTube/Vimeo links get special treatment with inline video players? — **Partially resolved.** For YouTube, links are detected client-side via `extractYouTubeVideoId()` in `$lib/utils/youtube.ts` and rendered as click-to-play embeds (`YouTubeEmbed.svelte` wrapping the `svelte-youtube-embed` package). The package shows a thumbnail with the video title (fetched via YouTube oEmbed API) and a play button; the iframe only loads on user click, using the privacy-enhanced `youtube-nocookie.com` domain. Client-side detection from message body text ensures embeds work independently of the backend link preview system. Vimeo support is not planned.
